...
class ORSNet(nn.Module):
    def __init__(self, n_feat, scale_orsnetfeats, kernel_size, reduction, act, bias, scale_unetfeats, num_cab):
        ...
        self.blur_caption = SP_blur_conv(n_feat + scale_orsnetfeats)
    ...
    def forward(self, x, encoder_outs, decoder_outs):
        # get sharp information, blur information
        blur = self.blur_caption(x)
        sharp_residual = x - blur

        x = self.orb1(sharp_residual)
        x = x + self.conv_enc1(encoder_outs[0]) + self.conv_dec1(decoder_outs[0])

        # get sharper result from sharp information
        x = self.orb2(x + sharp_residual)
        x = x + self.conv_enc2(self.up_enc1(encoder_outs[1])) + self.conv_dec2(self.up_dec1(decoder_outs[1]))

        # get sharper result from sharp information
        x = self.orb3(x + sharp_residual)
        x = x + self.conv_enc3(self.up_enc2(encoder_outs[2])) + self.conv_dec3(self.up_dec2(decoder_outs[2]))

        return {'result' : x, 'blur' : blur, 'sharp_residual' : sharp_residual}
...
class Reblurring_Module(nn.Module):
    ...
    '''
    def __init__(self, in_channels, Recurrent_times = 3, per_pix_kernel = 3):
        super().__init__() 
        self.kernel = per_pix_kernel
        self.recurrent_times = Recurrent_times
        self.BlurMask, self.convLSTM = nn.ModuleDict(), nn.ModuleDict() 
        self.channel = 3

        # transform blur information to per-pixel blur kernel
        self.PB = nn.Sequential(
                        nn.Conv2d(in_channels, per_pix_kernel**2, kernel_size = 3, padding=1),
                        nn.LeakyReLU(0.2),
                        )
    
        # ConvLSTM for exploit time information
        for i in range(self.recurrent_times-1):
            level = f'level_{i+1}'
            self.convLSTM[level] = ConvLSTM(input_dim = per_pix_kernel **2, hidden_dim=per_pix_kernel **2, kernel_size=(3, 3), num_layers=1, batch_first=True, bias=True, return_all_layers=True)

        # SA for per-pixel conv with correct weight to apply
    ...
    def generate_motion(self, perpixW):
        '''
        ...
        for i in range(self.recurrent_times - 1):
            level = f'level_{i+1}'
            layer_outputs, hidden_state = self.convLSTM[level](torch.unsqueeze(self.perpix[i], 1), hidden_state)
            self.perpix.append(torch.squeeze(layer_outputs[0], 1))

    def forward(self, blurInfo, feature):
        # PB : from blurInfo -> per-pixel w
        perpixW = self.PB(blurInfo)

        # expand with convlstm
        self.generate_motion(perpixW)

        # Reblur it, seperate S with Ms, Mb
        # then apply on blur
        # reblur += [sharp + blur]
        reblur = []  
        for i, perpix_kernel in enumerate(self.perpix):
            level = f'level_{i+1}'
...
class MPRBRRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=4, bias=False):
        ...
        self.RM = Reblurring_Module(n_feat + scale_orsnetfeats)

    def forward(self, x3_img, sharp):
        ...
        x3_dict = self.stage3_orsnet(x3_cat, feat2, res2)
        
        # get sharper result from sharp information
        stage3_img = self.tail(x3_dict['result'] + x3_dict['sharp_residual'])

        # get reblur restore
        Br = self.RM(x3_dict['blur'], stage3_img+x3_img)

        # get reblur sharp
        Bs = self.RM(x3_dict['blur'], sharp)

        return {'restore' : [stage3_img+x3_img, stage2_img, stage1_img], 'blur' : x3_dict['blur'], 
                'reblur_restore' : Br, 'reblur_sharp' : Bs}
