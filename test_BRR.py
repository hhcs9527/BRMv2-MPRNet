...
args.weights = os.path.join(model_dir, 'model_best.pth')
args.result_dir = result_dir

model_restoration = MPRBRRNet()#MPRBRRNet(n_feat=40, scale_unetfeats=40, scale_orsnetfeats=32)
...
with torch.no_grad():
    for ii, data_test in enumerate(tqdm(test_loader), 0):
        ...
        restored = model_restoration(input_, input_)
        restored = torch.clamp(restored['restore'][0],0,1)
